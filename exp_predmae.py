...
def get_costs(cur_reg,samples,sample_costs,profile_features,labels,cur_X_train,cur_y_train,num_to_profile,available_list,start_index,num_work,K):
  for j in range(start_index,start_index+num_work):
    ...
    reg = RandomForestRegressor(n_estimators=100).fit(np.concatenate((cur_X_train,new_X_train)),np.concatenate((cur_y_train,cur_reg.predict(new_X_train))))
...
json.dump(results.tolist(),open("exp_predmae_10sim.json","w"))
json.dump(this_train_sizes.tolist(),open("trainsize_exp_predmae_10sim.json","w"))
